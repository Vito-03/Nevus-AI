...
def process_metadata(directories, datasets):
    ...
    for dataset in datasets:
        ...
        if os.path.exists(directories[dataset]['nei_benigni']) and len(os.listdir(directories[dataset]['nei_benigni'])) > 0:
            continue  # Skip if no images in the folder

        # Percorso del file CSV
        metadata_file = os.path.join(metadata_path, dataset + ".csv") if metadata_path else None
        ...
        for index, row in data.iterrows():
            ...
            image_file = os.path.join(images_directory, f"{image_id}.jpg") if images_directory else None
            ...
            if diagnosis_1 == "benign":
                ...
                target_directory = directories[dataset]["nei_indeterminati"]
                counts["indeterminate"] += 1
                print(f"{dataset} - Diagnosi non riconosciuta '{diagnosis_1}' per l'immagine {image_id} - impostata a 'indeterminate'.")
            ...
            if target_directory and os.path.exists(target_directory):
                ...
                print(f"Errore: directory di destinazione '{target_directory}' non valida per l'immagine {image_id}.")
...
def create_pie_chart(counts, total_images, grafico_pie_path, exclude_indeterminate):
    ...
    wedges, texts, autotexts = plt.pie(
        ...
        autopct='%1.1f%%',
        startangle=90,
        colors = ["#4CAF50", "#F44336", "#FFC107"] if not exclude_indeterminate else ["#4CAF50", "#F44336"],
...
# Funzione per costruire la struttura dei dataset
def build_structure(base_path, datasets, subfolders):
    ...
    for dataset in datasets:
        ...
        for folder in subfolders:
            ...
            print(f"Checking if folder exists: {folder_path}")  # Debug: stampa il percorso
            ...
            if not os.path.exists(folder_path):
                try:
                    ...
                    print(f"Errore nella creazione della cartella '{folder_path}'. Dettagli: {e}")
...
def print_info(datasets, directories, subfolders):    
    for dataset in datasets:
        print(f"Totali in {dataset}: ", len([f for f in os.listdir(directories[dataset]['images'])]))
        for subfolder in subfolders[:3]:
            print(f'Totali in {subfolder} per {dataset}: ', len([f for f in os.listdir(directories[dataset][subfolder])]))

def create_charts(directories, datasets):
    for dataset in datasets:
        grafico_bar_path = os.path.join(directories[dataset]['diagrams'], dataset + "_diagram_bar.png")
        grafico_pie_path = os.path.join(directories[dataset]['diagrams'], dataset + "_diagram_pie.png")
        grafico_pie_path_with_indeterminate = os.path.join(directories[dataset]['diagrams'], dataset + "_diagram_pie_with_indeterminate.png")

        counts = {"benign": len([f for f in os.listdir(directories[dataset]['nei_benigni'])]), 
                  "malignant": len([f for f in os.listdir(directories[dataset]['nei_maligni'])]), 
                  "indeterminate": len([f for f in os.listdir(directories[dataset]['nei_indeterminati'])])}
        ...
        create_pie_chart(counts, total_images, grafico_pie_path_with_indeterminate, False)       
...
def main():    
    ...
    # sottocartelle
    subfolders = ["nei_benigni", "nei_maligni", "nei_indeterminati", "images", "metadata", "diagrams"]
